#!/usr/bin/env python3
"""Test pimpleFoam planarCouette case with multi-round error correction.

Purpose: Verify whether PIMPLE solver (pimpleFoam) also suffers from the
fvSolution missing pFinal issue documented for PISO solvers.

The reference fvSolution for pimpleFoam planarCouette includes pFinal:
    pFinal { $p; relTol 0; }

Without pFinal, pimpleFoam in OpenFOAM v10 may also fail.
...
Usage:
    ...
    python tests/test_pimpleFoam_couette_multirun.py
"""
...
def check_pfinal(case_dir: str) -> list[str]:
    """Check if fvSolution has pFinal entry for PIMPLE solver."""
    issues = []
    ...
    if "PIMPLE" not in content:
        issues.append("Missing PIMPLE sub-dictionary")
    return issues


def main():
    print("=" * 60)
    print("  pimpleFoam planarCouette — pFinal Issue Test")
    print("=" * 60)

    config = Config()

    user_requirement = """
Simulate a 2D planar Couette flow using pimpleFoam solver (PIMPLE algorithm) with laminar flow.

Setup:
- Solver: pimpleFoam
- Turbulence: laminar
- Kinematic viscosity nu = 1e-5 m^2/s
- Domain: 0.1m x 0.1m square, thin in z (0.01m) for 2D
- Mesh: 20x20 in x-y, 1 cell in z, uniform grading
- Top wall (movingWall) moves at (1 0 0) m/s
- Bottom wall (bottomWall) is no-slip
- Left/right boundaries are cyclic (cyc_half0 <-> cyc_half1)
- Front and back faces are empty (2D)
- Simulation: 0 to 25 seconds, dt = 0.005, write every 0.5s
- Also include a sigma field (initially uniform 0)

Boundary conditions:
- U: movingWall = fixedValue (1 0 0), bottomWall = noSlip, cyclic, empty
- p: movingWall = zeroGradient, bottomWall = zeroGradient, cyclic, empty
- sigma: movingWall = zeroGradient, bottomWall = zeroGradient, cyclic, empty
"""
    ...
    try:
        ...
        case_dir = os.path.abspath(resolve_case_dir(
            case_name="pimpleFoam_couette",
            case_dir="./output_pimpleFoam_couette_test",
        ...
        if pre_issues:
            ...
            print("  fvSolution complete (pFinal + PIMPLE present)")
